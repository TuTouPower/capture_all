...
## 6. 红线：标 ✅「已修复/已完成」前必须逐项验证

> 起因：commit `d8cf5ab`「P0.46-P0.55 标记全部已完成」一次性把 10 项 `⛔→✅` 批量翻牌，未逐项核验。结果 P0.53 露馅——标题翻成 ✅ 但状态行仍写「待修复」，且代码方案（`last_dir` 用绝对路径回填，chrome.downloads 只收相对路径）根本跑不通。批量改状态 = 没验证。
...
## 7. 红线：所有外部数据入口必须做 fault injection

> 起因：P0.58 / P0.59 反复出现「假设入参是合法值，运行时收到 undefined / null 触发 `.replace()` / `.toLowerCase()` 等 TypeError」的同一类崩溃。每次只补"上次踩过的坑"，没系统性测「输入边界」。这是结构性盲区，不是个案。
