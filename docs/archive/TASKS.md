...
### BUG-012：采集中 popup 数据标签统计延迟 1 秒刷新 ✅

**现象**：点击开始采集后离开 popup，再打开时数据标签统计全为 0，等 1 秒后才刷新。
**根因**：`start_timer()` 只设 setInterval，未立即调用 `refresh_counts()`。
**修复**：在 setInterval 前先调用 `refresh_counts()`。

---

## P0 · 功能缺陷（待修复）

### BUG-006：设置页采集上限 / 内联文本上限单位错误 ✅

**现象**：采集上限标签写"字节"，实际值很大（如 1048576），用户无法直观理解。
**要求**：
- 采集上限：单位改为 MB，input 的 step/显示值以 MB 为单位，存储仍为字节
- 内联文本上限：单位改为 KB，input 的 step/显示值以 KB 为单位，存储仍为字节
- 标签改为"采集上限 (MB)"、"内联文本上限 (KB)"

### BUG-007：诊断日志设置行重叠 ✅

**现象**：设置 → 诊断日志，"日志级别"的 seg 控件（5 个按钮：debug/info/warn/error/silent）与右侧"最大日志大小"input 在同一行重叠，seg 覆盖了 input。
**根因**：`.set-grid` 是 3 列 grid，seg 控件宽度超出单列，挤压右侧字段。
**要求**：修复布局，使日志级别和最大日志大小不重叠。可选方案：
- 日志级别 span 2 列
- 或单独一行放日志级别

### BUG-008：当前日志大小无输入框样式 ✅

**现象**："当前日志大小"下方的数字用裸 `<span>` 显示，没有边框，与同行其他 input 视觉不一致。
**要求**：改为只读 `<input>` 或带边框的容器，与"最大日志大小"视觉对齐。

### BUG-009：MCP bridge 默认关闭，应默认开启 ✅

**现象**：`constants.ts:62` 硬编码 `agent_bridge_enabled: false`，用户每次都要手动开启。
**根因**：默认值写死为 `false`。
**要求**：改为 `true`。
**测试缺失原因**：无测试验证 `DEFAULT_USER_CONFIG.agent_bridge_enabled` 的值。

### BUG-010：MCP 集成页"配置"按钮无反应 ✅

**现象**：主面板 → MCP / 集成，4 个卡片（MCP Bridge / 本地 Agent / Webhook / Issue 平台），点击"配置"按钮无任何反应。
**根因**：`render_integrations()` 中 `<button>` 未绑定 click handler。
**要求**：
- MCP Bridge "配置"：跳转到设置页 MCP Bridge 区域
- Webhook / Issue 平台：目前是纯占位（硬编码 `false`），要么隐藏，要么显示"即将推出"

### BUG-011：MCP 集成页 4 选项中 2 个是死占位 ✅

**现象**：Webhook 和 Issue 平台硬编码 `false`，无任何实现，用户点击"连接"无反应。
**要求**：未实现的功能不要显示为可交互卡片，改为灰色/禁用态或直接移除。
