...
### ✅ P0.59 多处 .replace / .textContent 未做 null/undefined 兜底，导出时崩溃
- **状态**：已修复 — 2026-06-13
...
- **测试为什么没发现**：所有相关单测都用 happy-path 合法输入：
  - `escape_html` 单测全用合法字符串。
  - `safe_request_id` 单测全用合法 id。
  - `base64_decoded_size` 单测全用合法 base64。
  - `logger.error()` 单测未测 Error 对象入参。

  系统性问题：**单测只补"上次踩过的坑"，未做系统性 fault injection**。每次修复后再触发新边界，是同一类思维盲区反复出现。
- **要求**：
  1. `escape_html` 改 `(s: unknown)` + `String(s ?? '')`。
  2. `safe_request_id` 改 `(id: string | undefined | null)`，fallback `'unknown'`。
  3. `base64_decoded_size` 改 `(b64: string | undefined | null)`，非 string 或空返回 0。
  4. `logger.write()` Error 对象转 `{ name, message, stack }`。
  5. 补 fault injection 单测：每个函数都测 undefined / null / 空字符串 / 合法值四档。
  6. 在 `docs/TEST_STRATEGY.md` 加「§N fault injection 红线」强制要求所有外部数据入口的单测必须覆盖 nullish 输入。
