...
// t202: bridge status from SW at wire; storage.onChanged syncs snapshot + label input.
// t204: module-level singleton for storage.onChanged (no leak across wire_settings; p054).
let _user_config_storage_listener_bound = false;

/** Test hook: reset singleton flag for isolation (mock addListener; no real remove). */
export function _reset_user_config_storage_listener_for_test(): void {
    _user_config_storage_listener_bound = false;
}

function ensure_user_config_storage_listener(): void {
    if (typeof chrome === 'undefined' || !chrome.storage?.onChanged) return;
    if (_user_config_storage_listener_bound) return;
    _user_config_storage_listener_bound = true;
    chrome.storage.onChanged.addListener((changes, area) => {
        if (area !== 'local' || !changes.user_config) return;
        set_user_config({ ...get_user_config(), ...(changes.user_config.newValue ?? {}) });
        // live DOM lookup — avoid closing over a stale content root
        const label_input = document.querySelector('#content [data-cfg="browser_label"]') as HTMLInputElement | null;
        if (label_input) {
...
function wire_bridge_status(c: HTMLElement): void {
    ...
    // query SW bridge status at wire (runtime chrome check; not module-level is_extension)
    if (typeof chrome !== 'undefined') {
        send_ui_message('get_bridge_status', {}).then((res) => {
            update_status(res.success ? (res.data ?? null) : null);
        }).catch(() => { /* keep default NotConnected on query failure */ });
    }
    ensure_user_config_storage_listener();
}
