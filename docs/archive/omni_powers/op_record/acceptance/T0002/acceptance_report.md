...
重验时间：2026-07-13 05:53:14 +0800

保留 Round 1 FAIL 历史。Round 2 使用正式脚本 `e2e/T0002/e2e-final-t0002.cjs` 加载本轮 `npm run build` 产物，Chromium 扩展真机执行。

| 验收标准 | 结果 | 本轮真实观测证据 |
|---|---|---|
| AC-1 | PASS | 点击 idx=0 可见标记后，`#tlPlayhead.style.left` 从 `49.5%` 更新为 `4.00031%`。 |
| AC-2 | PASS | 点击标记后 `.dt-insp` 可见，事件详情文本长度 156，包含 `tab_switch`、类型、时间、来源等字段。 |
| AC-3 | PASS | 依次点击 idx=0 与 idx=10；首个 inspector 显示 `tab_switch` 详情，第二次内容发生切换，正式脚本判定 `Content same: false`。 |
| AC-4 | PASS | 从 idx=0 标记位置拖动 100px 后 `.dt-insp` 不可见，playhead 更新至 `39.2117%`；仅移动播放头，未打开 inspector。 |

### 构建与交叉验证

- `npm run build`：PASS。
- 正式 E2E：AC-1~AC-4 全部 PASS。
- `npx vitest run tests/dashboard_timeline_marker.test.ts`：22 tests PASS；仅作为结构层交叉验证，不替代真机 E2E。

### Round 2 可用性判断

点击标记、详情切换、拖动分流均满足 AC-1~AC-4。Round 1 中 AC-3、AC-4 阻断问题已通过真机路径重验。
