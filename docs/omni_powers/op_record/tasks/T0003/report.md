...
| 文件 | 用途 |
|---|---|
| src/dashboard/dashboard.ts | 移除 NAV integrations 项、import、渲染分支，添加降级回退 |
| src/dashboard/dashboard_integrations.ts | 删除 render_integrations/wire_integrations，清理 import/export |
| src/dashboard/dashboard-pages.css | 删除 .integrations 相关 11 条 CSS 规则 |
| tests/integration_page.test.ts | 完全重写，15 条测试覆盖 AC-1/AC-3/AC-4 |
