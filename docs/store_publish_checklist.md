...
**已包含所有可直接复制粘贴的字段。截图由 Playwright 脚本自动捕获到 `store/screenshots/`。隐私政策直接指向本仓库 `PRIVACY.md`。**
...
6. 5 张 PNG 落地 `store/screenshots/`，均为 1280×800、真实扩展画面，非占位数据
...
- [ ] 执行 Playwright 截图脚本生成 `store/screenshots/`
