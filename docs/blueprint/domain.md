...
| browser_label | — | 人填的浏览器备注（可选，多实例时建议唯一）；AI 路由的 `target_label` 候选；非空且唯一时与 `instance_id` 等价可达 |
...
- **同 browser_label 再次 enroll 顶替旧实例**（label 非空时）：删除旧实例注册及命令队列（cancel_all 以 COMMAND_CANCELLED resolve），旧 instance_token 立即失效。heartbeat 携带 browser_label 同步变更。空 label 视为匿名，不顶替。
