...
- `src/content/dom_capture.ts` — 重命名为 input_event
- `src/content/content_script.ts`
- `src/content/storage_capture.ts`
- `src/content/xhr_fetch_capture.ts` — 合并到 network
- `src/content/network_hook.ts` — 合并到 network
- `src/background/network_capture.ts`
- `src/background/console_capture.ts` — 只采集 console
- `src/background/exception_capture.ts` — 独立为 error
