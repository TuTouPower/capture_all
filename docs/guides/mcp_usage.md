...
3. 启动 Bridge：进入本项目的 Claude Code 会话时 SessionStart hook 自动拉起；手动启动用 `node artifacts/bridge/bridge.mjs --port 17831`（`--port` 必须显式；未设 `CAPTURE_ALL_BRIDGE_TOKEN` 时自动生成并持久化到 `$XDG_RUNTIME_DIR/capture-all/bridge_token`，mode 0600）。Bridge 实例 registry 默认落盘到与 token 同目录的 `instances.json`（`CAPTURE_ALL_INSTANCES_FILE` 可覆盖路径），重启后恢复已注册浏览器，无需重新配对。
