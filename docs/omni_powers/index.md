...
| 命令 | 用途 |
|---|---|
| `/opintake "<需求>"` | 需求入口。spec 编写（含设计探索，task:spec 1:1）→ 闸门 A 批复 → 自动拆 task → tasks_list.json 就绪 |
| `/oprun` | 续跑执行。从 checkpoint 恢复，task 循环（review → merge → per-task 验收 → closer 收尾 → leader 自审写入 → 归档） |
| `/opstatus` | 状态查看。读 tasks_list.json + leader_checkpoint.md，渲染人类可读状态报告 |
