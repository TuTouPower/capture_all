...
| `src/extension/background/network_capture.ts` | 二进制不丢、记录 encoding/mime/byte_size、采集上限、base64 精确解码长度 | 修改 |
| `src/shared/archive_builder.ts` | 接收 CaptureData，组装 ZIP 包（页面侧） | 新建 |
| `src/shared/export_settings.ts` | `.zip` 扩展名 | 确认 |
| `src/extension/dashboard/dashboard.ts` | ZIP 导出入口 + 设置 UI | 修改 |
| `src/extension/popup/popup.ts` | ZIP 导出入口 | 修改 |
| `src/detail/detail.ts` | ZIP 导出入口 | 修改 |
| `docs/TASKS.md` | 任务记录 | 修改 |

删除项：
- `src/extension/background/archive_exporter.ts` — 不存在（spec v1 改页面侧后不再需要 SW 构建模块）。
...
- Modify: `src/extension/background/network_capture.ts:298-311`
...
`src/extension/background/network_capture.ts` 第 298-311 行替换为：
...
```bash
git add src/shared/types.ts src/extension/background/network_capture.ts tests/network_cdp.test.ts
...
**Files:**
- Modify: `src/extension/background/network_capture.ts`（NetworkCaptureConfig 字段）
- Modify: `src/shared/redaction.ts`
- Modify: `src/extension/background/external_cdp_bridge_client.ts`
- Modify: `src/extension/background/body_capture_coordinator.ts`
- Modify: `src/extension/popup/popup.ts`
- Modify: `src/extension/dashboard/dashboard.ts`（设置 UI）
...
- `src/extension/background/network_capture.ts`：`NetworkCaptureConfig` 内 `max_request_body_bytes`/`max_response_body_bytes` 两字段替换为 `max_body_capture_bytes: number; inline_text_max_bytes: number;`；所有读取处改 `config.max_body_capture_bytes`。
...
- `src/extension/background/external_cdp_bridge_client.ts`：`max_response_body_bytes` 形参改名 `max_body_capture_bytes`。
- `src/extension/background/body_capture_coordinator.ts`：传参 `config.max_response_body_bytes` 改 `config.max_body_capture_bytes`。
- `src/extension/popup/popup.ts`：`get_record_config()` 内 body 限制字段改为：
...
- `src/extension/dashboard/dashboard.ts`：设置页移除 `data-cfg="max_request_body_bytes"`/`max_response_body_bytes` 两个 input，新增：
...
**Files:**
- Modify: `src/extension/dashboard/dashboard.ts`
- Modify: `src/extension/popup/popup.ts`
...
describe('ZIP export entries', () => {
    it('dashboard sends get_capture_data and builds archive', () => {
        const src = read('src/extension/dashboard/dashboard.ts');
...
`src/extension/popup/popup.ts` 第 9-10 行 import 追加：
...
`src/extension/dashboard/dashboard.ts` import 追加：
