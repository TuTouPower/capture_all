...
`src/extension/devtools/`。提供 DevTools panel 注册入口。**设计优先级低于 popup 和 dashboard，目前无独立 UI 资产**。
...
`devtools.html` 加载 `devtools.ts`，后者通过 `chrome.devtools.panels.create` 注册一个面板，**指向 Dashboard**（`src/extension/dashboard/dashboard.html`）。此举复用 Dashboard 能力，避免维护空白 `devtools_panel.html`。
