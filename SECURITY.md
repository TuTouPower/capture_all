...
The Bridge is restricted to `127.0.0.1`. It rejects other bind addresses, requires a Bearer token on every non-health data endpoint, and restricts browser origins to Chrome extension origins. CORS preflight requests are checked by origin but do not carry application data. Authentication comparison uses a timing-safe hash comparison.

**Token model:** Two token types are used:
- **MCP token**: Protects Bridge MCP/CDP routes. Source: explicit CLI/env configuration, or Bridge-generated random token persisted to `0600` file. Stored as SHA-256 hash.
- **instance_token**: Per-extension enrollment token for extension data routes (`/extension/*`). Generated by Bridge on enroll, stored as hash, scoped to a single instance. Does not grant access to MCP/CDP routes.

Localhost binding does not protect against every process running on the same machine. Treat all tokens as secrets. Use tokens from explicit configuration or Bridge-generated random values (never hardcoded or example values). Keep `.mcp.json` local, and stop the Bridge when it is not needed.
