...
- `src/content/content_event_utils.ts` — content 侧事件公共字段填充（page_title / top_frame_url 等）。
...
- `src/content/content_script.ts` — 入口。
- `src/content/*_capture.ts` — 各 capture 模块（13 个）。
- `src/content/network_hook.ts` — body fallback。
- `src/content/content_event_utils.ts` — 公共字段。
