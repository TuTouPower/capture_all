...
项目根目录提供 `.mcp.json.example`。复制为只供本机使用的 `.mcp.json`，再填入扩展设置中的 Bridge Token：
...
1. `npm run build`
2. `cp .mcp.json.example .mcp.json`，填入扩展设置中的 Bridge Token
3. `CAPTURE_ALL_BRIDGE_TOKEN='<你的 Token>' node artifacts/bridge/bridge.mjs --port 17831 &`（后台持续运行；兼容参数 `--token` 同时存在时优先）
4. 重开 Claude Code 会话，MCP 工具自动加载
5. 对话中调用 `start_recording`、`list_captures`、`list_records` 等 MCP 工具
