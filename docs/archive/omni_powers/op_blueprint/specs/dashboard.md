...
`src/extension/dashboard/`。完整采集工作台，是用户可见的两个界面之一（另一个是 popup）。
...
go() 函数中 `integrations` 降级为 `captures`（`dashboard.ts:74`），`render_content()` 的 `else` 分支回退到 captures 页面（`dashboard.ts:84`）。旧 localStorage 残留 `_page === 'integrations'` 同样走降级路径，不会导致空白/报错。

模块拆分（P2 #11 重构）：`dashboard.ts`（路由 + 主框架）+ `dashboard_captures.ts`（采集列表）+ `dashboard_detail.ts`（详情）+ `dashboard_settings.ts`（设置）+ `dashboard_integrations.ts`（当前采集 / 导出任务）+ `dashboard_shared.ts`（共享工具）+ `icons.ts`。
...
### 3.1.2 缩放与 minimap 窗口平移

- 顶部 `#tlZoom` 控制可见时间窗口宽度；底部 `.tl-mm-window` 显示窗口在完整时间范围中的位置和宽度。
- `.tl-mm-window` 支持真实 pointer 水平拖动。拖动保持 zoom 值和窗口宽度不变，只改变窗口 `left`。
- playhead 始终跟随可平移窗口中心；时间标签、事件标记可见集合和 minimap 在拖动过程中实时同步。
- 窗口 `left` 钳制到 `[0, 100 - window_width]`，不能越过 minimap 左右边界。
- 100% 全览窗口无可平移空间，pointer 拖动为 no-op，不改变 playhead、zoom 或标记集合。
- 拖动使用 Pointer Capture；活动拖动期间只接受当前 `pointerId`，拒绝其他指针启动第二次拖动；指针移出红色窗口或 minimap 后仍持续响应，`pointerup` / `pointercancel` / capture 丢失时清理拖动态。
- minimap 拖动不打开、关闭或重渲染 inspector。
...
- `src/extension/dashboard/dashboard.html` / `dashboard.ts` / `*.css`（shell + pages + detail + views 四套）。
- `src/extension/dashboard/dashboard_captures.ts` / `dashboard_detail.ts` / `dashboard_settings.ts` / `dashboard_integrations.ts` / `dashboard_shared.ts`。
- `src/extension/dashboard/sidebar_resize.ts` — 可拉伸侧边栏（FEAT-002）。
- `src/extension/dashboard/icons.ts` — 图标。
